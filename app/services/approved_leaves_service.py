...
from app.repositories.get_approved_leaves_by_id_repo import get_approved_leaves_by_id
from app.repositories.get_all_approved_leaves_repo import get_all_approved_leaves
from app.repositories.get_employee_names_by_ids_repo import get_employee_names_by_ids

async def get_approved_leaves_service(
    db: AsyncSession,
    current_user: dict,
    target_employee_id: Optional[int]
) -> Dict[str, Any]:
    
    approved_leaves = [] # Store the raw SQLAlchemy objects here first
    
    # Safely extract the current user's ID to avoid dictionary key errors
    ...
    if current_user.get("role") == "approver":
        if target_employee_id is not None:
            approved_leaves = await get_approved_leaves_by_id(db, employee_id=target_employee_id)
            if not approved_leaves:
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND,
                    detail=f"No approved leaves found for employee ID {target_employee_id}."
                )
        else:
            approved_leaves = await get_all_approved_leaves(db)
            if not approved_leaves:
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND,
                    detail="No approved leaves found for any employee."
                )
    else:
        if target_employee_id is None:
            ...
            raise HTTPException(
                ...
                detail="You do not have permission to view other employees' approved leaves."
            )
    ...
    return {
        "status": "success",
        "message": f"Retrieved {len(hydrated_leaves)} approved leave record(s).",
